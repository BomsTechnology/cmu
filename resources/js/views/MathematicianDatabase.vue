<script setup>
        ...
        const { members, getMembers, loading, errors } =
            useMembers();
        const keyWords = ref("");
        ...
        get() { return members.value.filter((member) => {
...
<div class="lg:flex py-4 md:px-10">
    ...
    <section class="max-w-4xl py-6 px-4 mx-auto text-center bg-white rounded-md border border-gray-300 dark:bg-gray-800">   
        <h2 class="text-lg font-semibold text-blue-600 capitalize dark:text-white">Filter Mathematician</h2>     
        <form>
            <div class="mt-4">
                <div>
                    <input id="" type="search" v-model="keyWords" placeholder="Keywords" class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-200 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-400 focus:ring-blue-300 focus:ring-opacity-40 dark:focus:border-blue-300 focus:outline-none focus:ring">
                </div>
    ...
    <div class="block lg:w-3/4">
            <div v-if="filteredMember.length != 0" class="grid grid-cols-1 gap-8 mt-8 xl:mt-12 xl:gap-12 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 px-6">
                 <router-link v-for="member in filteredMember" :key="member.id" :to="{name:'account', params: {name: member.firstname.toLowerCase(), id:member.id}}" v class="flex flex-col items-center pt-9 text-center space-y-1 rounded-xl dark:bg-gray-800 border border-gray-300">
                        <img class="object-cover rounded-full h-36 w-36" :src="member.avatar" v-if="member.avatar">
                        <UserCircleIcon v-else class="h-36 w-36 text-gray-700"/>

                        <h4 class="text-gray-700 font-bold capitalize dark:text-gray-300"> {{ member.firstname }} {{ member.lastname }}</h4>

                        <h6 class="text-xs font-extralight italic">{{ member.field_research }}</h6>

                        <h4 class="text-sm font-light pb-4">{{ member.university.name }}</h4>
                </router-link>      
            </div>
